...
export default class NodesMapper {
  ...
  static transformMdsNodes(nodes, avatarData) {
    try {
      const copy = [];
      nodes.forEach((node) => {
        const superDrive = find(node.services, srv =>
          srv.serviceType === NodesMapper.matchServiceType);
        if (superDrive) {
          const name = find(node.attributes, att => att.name === 'name');
          const os = find(node.characteristics, att => att.name === 'os');
          copy.push({
            id: node.nodeId || node.id,
            accountId: node.accountId || node.account.id,
            name: name && name.value,
            routing: NodesMapper.getRouting(node),
            os: os && os.value,
            avatar: avatarData,
            location: node.location,
            url: NodesMapper.getUrl(node),
          });
        }
      });
